...
const initMeshHandler = (scene: Scene, events: Events) => {
    ...
    const reflectionTimers = new WeakMap<MeshElement, number>();

    const scheduleReflectionCapture = (mesh: MeshElement, delay = 350) => {
        const oldTimer = reflectionTimers.get(mesh);
        if (oldTimer !== undefined) {
            window.clearTimeout(oldTimer);
        }

        const timer = window.setTimeout(() => {
            reflectionTimers.delete(mesh);
            void mesh.captureReflection();
        }, delay);
        reflectionTimers.set(mesh, timer);
    };

    const registerMesh = (mesh: MeshElement) => {
        meshList.push(mesh);
        events.fire('mesh.added', mesh);
        // Auto-capture after the scene has had a few frames to render.
        // The probe is used as the model's environment reflection map.
        scheduleReflectionCapture(mesh, 450);
    };
