...
class MeshElement extends Element {
     ...
     *
     * FIX: When preset === 'original', we now inject the envAtlas directly into
     * the GLB's own StandardMaterial instances (instead of skipping them).
     * This preserves all original textures, PBR values, and clearcoat —
     * while adding the scene-captured IBL environment for real reflection.
     */
    async captureReflection() {
        ...
        if (atlas) {
            ...
            if (!this._useOriginalMaterials) {
                // Custom preset path — rebuild our StandardMaterial and apply everywhere
                this._buildMaterial();
                this._walkAndApply(this.pivot);
            } else {
                // 'original' preset path — inject envAtlas into the GLB's own materials.
                // We do NOT replace the materials — just set envAtlas + useSkybox = false
                // so PlayCanvas's PBR shader uses our captured IBL for reflections.
                this._applyEnvToOriginalMaterials(this.pivot, atlas);
            }
    ...
    setMaterialOptions(opts: Partial<MeshMaterialOptions>) {
        ...
        // Switching back to 'original' → restore envAtlas injection on GLB materials
        if (opts.preset === 'original') {
            this._useOriginalMaterials = true;
            // Re-apply the cached envAtlas to the original GLB materials
            if (this.pivot && this._envAtlas) {
                this._applyEnvToOriginalMaterials(this.pivot, this._envAtlas);
            }
        }
    ...
    /**
     * Inject a captured envAtlas into GLB's own StandardMaterial instances.
     *
     * This is the key fix for 'original' preset reflection:
     * - We do NOT replace or clone the material
     * - We set envAtlas + useSkybox = false so PlayCanvas's own PBR shader
     *   uses our captured 3DGS scene IBL for specular reflection
     * - All original properties (metalness, roughness, textures, clearcoat,
     *   baseColor, normalMap, etc.) remain exactly as the GLB author intended
     *
     * We track which materials we have already patched using a WeakSet so that
     * shared materials (same mat on multiple mesh instances) are only updated once.
     */
    private _applyEnvToOriginalMaterials(entity: Entity, atlas: Texture) {
        const seen = new WeakSet<StandardMaterial>();
        this._walkApplyEnv(entity, atlas, seen);
    }

    private _walkApplyEnv(entity: Entity, atlas: Texture, seen: WeakSet<StandardMaterial>) {
        const r = (entity as any).render;
        if (r?.meshInstances) {
            for (const mi of r.meshInstances) {
                ...
                // Only patch StandardMaterial — skip any custom shader materials
                if (typeof mat.envAtlas === 'undefined') continue;

                mat.envAtlas = atlas;
                mat.useSkybox = false;

                // For very rough materials (roughness ≥ 0.9), the specular lobe is so
                // wide that IBL reflection is barely visible anyway — skip update() call
                // to save CPU. For all others, we need update() to recompile the shader
                // with the new envAtlas path.
                const gloss = (mat as any).gloss ?? (1 - ((mat as any).roughness ?? 1));
                if (gloss > 0.05) {
                    mat.update();
                }
            }
        ...
        for (let i = 0; i < entity.children.length; i++) {
            this._walkApplyEnv(entity.children[i] as Entity, atlas, seen);
        }
    }
